...
class MiniImageNet(Dataset):
    ...
    def __getitem__(self, item):
        # Open the image
        instance = Image.open(self.datasetid_to_filepath[item]).convert('RGB')  # Convert image to RGB
    ...
    def index_subset(subset):
        ...
        for root, folders, files in os.walk(DATA_PATH + '/miniImageNet/images_{}/'.format(subset)):
            subset_len += len([f for f in files if f.endswith('.JPEG')])  # Check for .JPEG files
        ...
        for root, folders, files in os.walk(DATA_PATH + '/miniImageNet/images_{}/'.format(subset)):
            ...
            for f in files:
                if f.endswith('.JPEG'):  # Ensure file is a .JPEG
                    progress_bar.update(1)
                    images.append({
                        'subset': subset,
                        'class_name': class_name,
...
class YCBObjectSet(Dataset):
    def __init__(self, subset):
        """Dataset class representing Yale–CMU–Berkeley Object and Model Set
    ...
    def index_subset(subset):
        ...
        # Quick first pass to find total for tqdm bar
        subset_len = 0
        for root, folders, files in os.walk(os.path.join(DATA_PATH, 'YCBObjectSet2', 'images_{}'.format(subset))):
            subset_len += len([f for f in files if f.endswith('.jpg') or f.endswith('.jpeg') or f.endswith('.png')])
