...
class FewShotClassifier(nn.Module):
    def __init__(self, num_input_channels: int, k_way: int, fc_layer_size: int = None):
        ...
        # Si fc_layer_size se proporciona, úsalo; de lo contrario, calcula dinámicamente
        if fc_layer_size is None:
            with torch.no_grad():
                dummy_input = torch.zeros(1, num_input_channels, 84, 84)
                dummy_output = self.conv4(self.conv3(self.conv2(self.conv1(dummy_input))))
                fc_layer_size = dummy_output.view(1, -1).size(1)

        self.logits = nn.Linear(fc_layer_size, k_way)
...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv3 = conv_block(64, 64)
        self.conv4 = conv_block(64, 64)
        self.flatten = Flatten()  # Usando Flatten aquí
        self.fc = nn.Linear(64 * 5 * 5, num_classes)  # Ajuste a 1600 basado en las dimensiones de salida
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.conv4(x)
        x = self.flatten(x)  # Aplicamos Flatten para aplanar el tensor
        x = self.fc(x)  # Pasamos el tensor a la capa lineal
        return x
